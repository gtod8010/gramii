...
import { useUser } from "@/hooks/useUser"; // useUser 훅 임포트 (경로 확인 필요)

export default function UserDropdown() {
  const [isOpen, setIsOpen] = useState(false);
  const { user, isLoading, logout } = useUser(); // useUser 훅 사용

  // 외부 클릭 시 드롭다운 닫기 (선택적 개선 사항)
  useEffect(() => {
  ...
  // 로딩 중이거나 사용자가 없으면 드롭다운을 표시하지 않거나, 스켈레톤 UI를 보여줄 수 있음
  if (isLoading) {
    return (
      <div className="relative">
        <div className="flex items-center">
          <span className="mr-3 overflow-hidden rounded-full h-11 w-11 bg-gray-200 dark:bg-gray-700 animate-pulse"></span>
          <span className="block mr-1 h-5 w-20 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></span>
        </div>
      </div>
    );
  }

  // 사용자가 로그인하지 않은 상태 (선택적: 로그인 버튼 표시 또는 아무것도 표시 안함)
  if (!user) {
    return (

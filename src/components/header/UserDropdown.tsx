...
export default function UserDropdown() {
      ...
      <div className="relative">
        <div className="flex items-center">
          ...
          <div className="flex-grow">
            <span className="block mr-1 h-5 w-20 bg-gray-200 dark:bg-gray-700 rounded animate-pulse mb-1"></span>
            <span className="block mr-1 h-3 w-24 bg-gray-200 dark:bg-gray-700 rounded animate-pulse"></span>
          </div>
        </div>
      </div>
    );
  ...
  if (!user) {
    return (
      <Link href="/login" className="px-4 py-2 text-sm font-medium text-gray-700 rounded-lg hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700">
        로그인
      </Link>
    );
  }
  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center text-gray-700 dark:text-gray-400 dropdown-toggle w-auto"
      >
        <div className="text-left mr-2 flex-grow" style={{ minWidth: '180px' }}> 
          ...
          <div className="flex items-center mt-0.5"> 
            ...
            {user.admin_referral_code && (
              <div 
                className={`flex items-center text-xs text-blue-500 dark:text-blue-400 cursor-pointer hover:underline ${user.points !== undefined ? 'ml-2' : ''}`}
                onClick={(e) => { 
                  e.stopPropagation(); // 드롭다운 열림 방지
                  handleCopyReferralCode();
                }}
                title="클릭하여 추천인 코드 복사"
              >
                추천인: {user.admin_referral_code}
                {copied ? (
                  <CheckIcon className="w-3 h-3 ml-1 text-green-500" />
                ) : (
                  <ClipboardIcon className="w-3 h-3 ml-1" />
                )}
              </div>
            )}
          </div>
           ...
           {user.admin_referral_code && (
            <div className="mt-0.5 flex items-center">
                <span className="text-theme-xs text-gray-500 dark:text-gray-400">
                    추천인 코드: {user.admin_referral_code}
                </span>
                <button 
                    onClick={handleCopyReferralCode}
                    className="ml-2 p-0.5 rounded hover:bg-gray-200 dark:hover:bg-gray-700"
                    title="추천인 코드 복사"
                >
                    {copied ? (
                        <CheckIcon className="w-3 h-3 text-green-500" />
                    ) : (
                        <ClipboardIcon className="w-3 h-3 text-gray-500 dark:text-gray-400" />
                    )}
                </button>
            </div>
          )}
